...
def effective_transfer_entropy(X,Y,delay=1,gaussian_sigma=None,repeat=1):
    sum=0
    X = np.array(X)
    Y = np.array(Y)
    for i in range(repeat):
        ...
        sum+=transfer_entropy(X=X,Y=Y_shuffle,delay=delay,gaussian_sigma=gaussian_sigma)
    sum = sum/repeat
    return transfer_entropy(X,Y,delay=delay,gaussian_sigma=gaussian_sigma)-sum
...
def transfer_entropy(X,Y,delay=1,gaussian_sigma=None):
    ...
    if binX>100: binX = 100
    if binY>100: binY = 100
